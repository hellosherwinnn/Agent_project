...
LLM_MODEL = os.getenv("LLM_MODEL_ID") # 已修正为读取 .env 中的 LLM_MODEL_ID

# 动态修补 OpenAI 同步客户端以去除 "name" 字段 (专门针对 MiniMax 和 Qwen 不支持角色的 name 场景)
...
Completions.create = custom_create

model = ModelFactory.create(
    model_platform=ModelPlatformType.OPENAI, # 改为 OPENAI 平台以适应普通兼容 API 比如 MiniMax/Gemini
